...
EXTENSION_ROOT = PROJECT_ROOT / "extensions" / "agile"
PACKAGE_PARENT = EXTENSION_ROOT / "scripts"
CLI = EXTENSION_ROOT / "scripts" / "agile" / "cli.py"

sys.path.insert(0, str(PACKAGE_PARENT))

from agile.coverage import calculate_coverage
from agile.errors import DomainError, InfrastructureError
from agile.graph import build_graph
from agile.io import load_json, safe_path
from agile.ledger import append_event, read_ledger
from agile.models import FeatureTrace, Relationship, Requirement
from agile.registry import allocate_id, transition_requirement
from agile.trace import validate_trace_pair, write_trace
...
def setup_verified_agile_sprint(root: Path) -> Path:
    ...
    plan_input = root / ".specify" / "agile" / "agile-plan-input.md"
...
"""
    ...
    def test_agile_plan_approval_rejects_unapproved_requirements(
        ...
        plan_input = tmp_path / ".specify" / "agile" / "plan.md"
        plan_input.parent.mkdir()
...
class TestPackageSurface:
    ...
    def test_commands_resolve_extension_local_tool_calls(self):
        ...
        for command in manifest.commands:
            ...
            assert (
                ".specify/extensions/agile/scripts/bash/"
                "agile.sh"
            ) in rendered
    ...
    def test_codex_registers_agile_commands_as_skills(self, tmp_path: Path):
        ...
        for name in ("architech", "kickoff", "breakdown", "sprint-check", "sprint-verify"):
            ...
            assert "agile.sh" in skill.read_text()
    ...
    def test_install_copies_deterministic_package(self, tmp_path: Path):
        ...
        installed = tmp_path / ".specify" / "extensions" / "agile"
        assert manifest.id == "agile"
        assert (installed / "scripts" / "agile" / "cli.py").is_file()
